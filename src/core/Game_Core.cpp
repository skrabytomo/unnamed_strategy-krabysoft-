...
bool Game::init(const std::string& title, int width, int height, bool hidden)
{
    ...
    m_window = SDL_CreateWindow(title.c_str(),
        ...
        SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE
            | (hidden ? SDL_WINDOW_HIDDEN : SDL_WINDOW_MAXIMIZED));
    ...
    // Vsync on a hidden, never-presented window can starve the whole loop —
    // some compositors never fire the swap-interval callback for a window
    // that isn't actually being shown, so the --watch-ai-test path runs
    // uncapped instead of blocking on a vsync that will never arrive.
    ...
    // 12.65 = 40 / sqrt(10), paired with the ~sqrt(10)x radius bump in
    // mapRadius() (HexMap.h) — ~10x denser hex grid at roughly the same
    // on-screen reach as before, instead of just a physically bigger map.
    if (!m_hexRenderer.init(12.65f, m_basePath,
    ...
    for (int i = 0; i < NUM_FACTIONS; ++i) {
        ...
        // mipmap=true: these 1024² illustrations are shown at 40–90px, so
        // trilinear minification keeps them crisp instead of shimmery.
        m_portraitTex[i].load(m_basePath + rel, false, false, false, false, false, /*mipmap*/true);

        m_portraitVarCount[i] = 0;
        for (int v = 1; v <= MAX_PORTRAIT_VARS; ++v) {
            char vb[80];
            std::snprintf(vb, sizeof(vb), "assets/portraits/faction_%d_%d.png", i, v);
            if (m_portraitVar[i][v - 1].load(m_basePath + vb, false, false, false, false, /*quiet*/true, /*mipmap*/true))
        ...
        m_crestTex[i].load(m_basePath + cb, false, false, false, false, /*quiet*/true, /*mipmap*/true);
    }
    ...
    for (int i = 0; i < NUM_FACTIONS; ++i) {
        // Prefer the curated variant portraits; only fall back to the basic
        // faction_F.png when a faction has none (the basics clash with the new
        // style, so they're dropped from rotation once real portraits exist).
        if (m_portraitVarCount[i] > 0) {
            for (int v = 0; v < m_portraitVarCount[i]; ++v)
                if (m_portraitVar[i][v].ok())
                    m_worldHUD.addPortrait(i, (ImTextureID)(uintptr_t)m_portraitVar[i][v].id());
        } else if (m_portraitTex[i].ok()) {
        ...
        m_worldHUD.setMaxBuildings(i,
            static_cast<int>(m_registry.getBuildingsForFaction(static_cast<FactionId>(i)).size()));
    }
...
void Game::saveGame(const std::string& customName)
{
    ...
    data.aiResourceAmounts.clear();
    for (const auto& r : m_aiResources) data.aiResourceAmounts.push_back(r.amounts);
    // Legacy combined field, kept only so older builds could still parse this
    // save; nothing in this codebase reads it back anymore.
    data.enemyResourceAmounts = {};
    for (const auto& r : m_aiResources)
        for (int i = 0; i < RESOURCE_COUNT; ++i) data.enemyResourceAmounts[i] += r.amounts[i];
    data.playerTeam = m_playerTeam;
...
bool Game::loadGameApply(GameSaveData& data)
{
    ...
    // Enemy heroes don't persist ownerId explicitly in the save format — it's
    // always derivable from creation/vector order (index i -> ownerId
    // numHumanPlayers+1+i, same as Town::ownerId for that zone), so just
    // re-derive it here rather than touching HeroSave/JSON for one field.
    for (size_t i = 0; i < m_enemyHeroes.size(); ++i)
        m_enemyHeroes[i].ownerId = static_cast<uint32_t>(m_numHumanPlayers + 1 + (int)i);

    // Per-AI-player resource pools (v7+). Older saves (or a player-count
    // mismatch from manual save editing) get fresh per-player defaults instead
    // of guessing how to split whatever the old shared pool had.
    if (data.aiResourceAmounts.size() == m_enemyHeroes.size()) {
        m_aiResources.clear();
        for (const auto& amounts : data.aiResourceAmounts) {
            Resources r; r.amounts = amounts;
            m_aiResources.push_back(r);
        }
    } else {
        m_aiResources.assign(m_enemyHeroes.size(), Resources{});
        for (auto& r : m_aiResources) {
            r.set(ResourceType::Gold, 5000);
            r.set(ResourceType::Iron,   20);
        }
    }

    // Alliance teams (v7+). Older saves have none — everyone is a free
    // agent, identical to pre-alliance behaviour.
    m_playerTeam = data.playerTeam;
...
void Game::startNewGame()
{
    ...
    int  sortedFaction[MAX_SETUP_SLOTS], sortedBonus[MAX_SETUP_SLOTS], sortedTeam[MAX_SETUP_SLOTS];
    int  numHumansResolved = 0;
    {
        ...
        // Team assignment follows the SAME humans-first/bots-after reorder,
        // index-aligned with ownerId (slot idx -> ownerId idx+1).
        m_playerTeam.assign(sortedTeam, sortedTeam + slotCount);
    }
    ...
    static constexpr MapShape kMapShapes[] = {
        MapShape::Hexagon, MapShape::JebusCross, MapShape::JebusCross3, MapShape::Ring
    };
    ...
    wgp.shape       = kMapShapes[std::clamp(m_newGameMapShape, 0, 3)];
    ...
    // Was hardcoded to "i <= 3" back when the lobby capped out at 4 total
    // slots; with up to MAX_SETUP_SLOTS now, that silently left bot slots
    // 5-8 with a town but no AI hero to run it. Use the real slot count.
    for (int i = aiStartIdx; i < static_cast<int>(wgResult.startPositions.size()) && i < slotCount; ++i) {
        ...
        eHero.ownerId  = static_cast<uint32_t>(i + 1);  // matches Town::ownerId for this same zone
        eHero.name     = eName;
        eHero.faction  = ef;
    ...
    // Each AI player gets its own independent economy now, starting with the
    // same resources a human gets (5000g + 20 iron) — not one shared pool
    // sized by headcount like before.
    m_aiResources.assign(m_enemyHeroes.size(), Resources{});
    for (auto& res : m_aiResources) {
        res.set(ResourceType::Gold, 5000);
        res.set(ResourceType::Iron,   20);
    }
    ...
    for (int i = 0; i < static_cast<int>(wgResult.towns.size()); ++i) {
        ...
        if (i == 0) {
            ...
            // Assign enemy town to the corresponding AI hero. The hero's
            // Hero::id uses a different numbering scheme (99+i) than
            // Town::ownerId/Hero::ownerId (player-slot ids, i+1) — this used
            // to assign wt.ownerId = aiId (the hero-id value), which put
            // every AI town's ownerId completely out of the player-slot
            // range. That silently broke aiResources(t.ownerId) (always
            // out-of-bounds -> a dummy pool reset to zero on every access)
            // and every t.ownerId == eHero.ownerId comparison for that
            // player's own town, leaving the AI town's economy permanently
            // dead from turn 1 (confirmed live: 4 of 5 AI players sat at 0
            // gold and never built/recruited/re-hired a hero for 48+ weeks).
            ...
            if (assignedToAI) {
                wt.ownerId = static_cast<uint32_t>(i + 1);  // matches eHero.ownerId for this zone
    ...
    // +1500 gold. Humans and bots alike — same rules for everyone.
    {
        ...
        for (int si = 0; si < slotCount; ++si) {
            ...
            if (si == 0) {
                ...
                pool = &aiResources(static_cast<uint32_t>(si + 1));
...
void Game::autoStartWatchAI(int playerCount, int shape, int size)
{
    m_setupPlayerCount = std::clamp(playerCount, 2, MAX_SETUP_SLOTS);
    m_newGameMapShape  = std::clamp(shape, 0, 3);
    m_newGameMapSize   = std::clamp(size, 0, 3);
    for (int s = 0; s < MAX_SETUP_SLOTS; ++s) {
        m_slotType[s]    = 1;   // everyone a bot — Watch AI drives slot 0 too
        m_slotFaction[s] = 9;   // random faction
        m_slotBonus[s]   = 2;
        m_slotTeam[s]    = 0;   // free agent by default
    }
    // Slots 0/1 allied, so alliance behaviour gets exercised alongside FFA
    // hostility among the rest.
    if (m_setupPlayerCount >= 2) { m_slotTeam[0] = 1; m_slotTeam[1] = 1; }
...
bool Game::initImGui()
{
    ...
    // Default ProggyClean font only covers ASCII/Latin-1, so any dash/quote
    // glyph outside that (em dash, curly quotes) rendered as "?". Load a real
    // TTF with those glyph ranges included to fix it everywhere at once.
    {
        ImGuiIO& fio = ImGui::GetIO();
        static const ImWchar ranges[] = {
            0x0020, 0x00FF, // Basic Latin + Latin-1 Supplement (ImGui default range)
            0x2010, 0x2027, // General Punctuation: hyphens, dashes, curly quotes
            0,
        };
        std::string fontPath = m_basePath + "assets/fonts/DroidSans.ttf";
        if (!fio.Fonts->AddFontFromFileTTF(fontPath.c_str(), 15.0f, nullptr, ranges))
