                       ...
                       const std::vector<CombatUnit>& enemyUnits)
{
    ...
    if (isSiege) {
        ...
        bool hasBastion = siegeTown ? siegeTown->hasBuilding(BID::BASTION)
                                    : (simSiege && m_simBastion);
        if (hasBastion) { wallHP = wallHP * 5 / 4; gateHP = gateHP * 5 / 4; }
        // Fortify action (Game_Town.cpp): extra wall/gate HP and tower damage
        // for this siege only. Was set but never read anywhere — the defender
        // got the DEF bonus below but not these two promised bonuses.
        int fortifyTowerAtk = 0;
        if (siegeTown && siegeTown->fortifyWallBonus > 0) {
            wallHP += siegeTown->fortifyWallBonus;
            gateHP += siegeTown->fortifyWallBonus;
        }
        if (siegeTown) fortifyTowerAtk = siegeTown->fortifyTowerBonus;
        ...
        for (int ti = 0; ti < 2; ++ti) {
            ...
            CombatUnit tower;
            tower.name         = td.name;
            tower.attack       = td.atk + fortifyTowerAtk;
            tower.defense      = 10;
            tower.hp           = tower.maxHp = td.hp;
            tower.count        = 1;
            tower.speed        = 1;
    ...
    if (playerWon) {
        ...
        if (m_lastCombatEnemyId != 0) {
            ...
            if (m_enemyHeroes.empty()) {
                ...
                for (auto& recruitTown : m_towns) {
                    if (recruitTown.ownerId <= static_cast<uint32_t>(m_numHumanPlayers)) continue;
                    // Each AI player pays from its own pool now — check per town
                    // owner instead of one shared gate before the loop.
                    if (aiResources(recruitTown.ownerId).get(ResourceType::Gold) < EMERG_HIRE_COST) continue;
                    // Camped by a hostile hero? Recruiting is a gold donation.
                    bool camped = false;
                    for (const auto& hh : m_heroes)
                        if (HexGrid::distance(hh.pos, recruitTown.pos) <= 4) { camped = true; break; }
                    ...
                    newHero.name     = kEmergNames[newId % 4];
                    newHero.movePool = newHero.maxMove;
                    aiResources(recruitTown.ownerId).add(ResourceType::Gold, -EMERG_HIRE_COST);
                    // Tavern retinue (T1 growth + T2 growth/3, included in fee)
                    for (int tier : {1, 2}) {
                        int growth = 0;
                        for (const auto& bd : m_registry.buildings()) {
        ...
        // After a garrison victory, drop the player into the captured town —
        // except in Watch AI mode, where there's no one to click through the
        // town UI and open it there would hard-freeze the sim forever (the
        // world-map update loop, including the auto-dismiss-modals logic
        // just above, never runs while m_state == GameState::Town).
        if (captured) {
            if (m_watchingAI) {
